...
export default function UserListedMovies() {
  ...
  // Safe data fetching - FIXED: Prevent multiple fetches
  useEffect(() => {
    if (email && !hasFetched) {
      setIsLoading(true);
      setHasFetched(true);
  ...
  }, [email, dispatch, hasFetched]);

  // Filter movies based on search - FIXED: Added null check
  useEffect(() => {
    if (movies && Array.isArray(movies)) {
      const filtered = movies.filter(movie =>
        movie && movie.name?.toLowerCase().includes(searchTerm.toLowerCase())
      );
  ...
  const getStats = () => {
    if (!movies || !Array.isArray(movies)) return { movies: 0, tvShows: 0, total: 0 };
    
    const validMovies = movies.filter(movie => movie && typeof movie === 'object');
    const moviesCount = validMovies.filter(movie => movie.type === 'movie').length;
    const tvShowsCount = validMovies.filter(movie => movie.type === 'tv').length;
  ...
  // Safe render function - FIXED: Added proper array validation
  const renderContent = () => {
    if (isLoading) {
      return (
        <LoadingContainer>
    ...
    // FIXED: Added comprehensive validation for movies array
    const isValidMoviesArray = movies && Array.isArray(movies) && movies.length > 0;
    const validMovies = isValidMoviesArray ? movies.filter(movie => 
      movie && typeof movie === 'object' && movie.id
    ) : [];

    const displayMovies = searchTerm ? filteredMovies : validMovies;
    const hasDisplayMovies = displayMovies && Array.isArray(displayMovies) && displayMovies.length > 0;
...
// Animations and styled components remain unchanged
...
// Styled Components (keep all your existing styled components exactly the same)
